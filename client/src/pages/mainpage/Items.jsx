...
import { Trash2, Plus, Edit2, X, UtensilsCrossed, Calendar } from "lucide-react";
...
export default function Items() {
  ...
  useEffect(() => {
    ...
    fetch('/api/groceries-master')
      .then(res => res.json())
      .then(data => setGroceryMaster(data.map(g => ({ id: g.id, name: g.grocery_name, unit: g.unit }))));
  ...
  const validateForm = () => {
    ...
    if (!formData.quantityPrepared || isNaN(formData.quantityPrepared) || Number(formData.quantityPrepared) <= 0) newErrors.quantityPrepared = "Quantity prepared must be > 0";
    if (!formData.preparedDate) newErrors.preparedDate = "Prepared date is required";
    if (!formData.pricePerItem || isNaN(formData.pricePerItem) || Number(formData.pricePerItem) < 0) newErrors.pricePerItem = "Price must be >= 0";
    formData.groceriesUsed.forEach((g, idx) => {
      if (!g.groceryId) newErrors[`grocery_${idx}`] = "Select grocery";
      if (!g.quantity || isNaN(g.quantity) || Number(g.quantity) <= 0) newErrors[`quantity_${idx}`] = "Enter valid quantity";
    });
    setErrors(newErrors);
  ...
  const handleAddGrocery = () => {
    setFormData({
      ...formData,
      groceriesUsed: [...formData.groceriesUsed, { groceryId: "", quantity: "", unit: "" }]
    });
  };
      ...
      <div>
        ...
        <div className="mt-2 flex gap-2 justify-end">
          <button
            ...
            onClick={async () => {
              try {
                const res = await fetch(`/api/prepared-items/${id}`, { method: 'DELETE' });
                if (!res.ok) {
                  const errorData = await res.json();
      ...
      <div>
        ...
        <div className="mt-2 flex gap-2 justify-end">
          <button
            ...
            onClick={async () => {
              try {
                await Promise.all(selectedRows.map(id => fetch(`/api/prepared-items/${id}`, { method: 'DELETE' })));
                setItems(items => items.filter(item => !selectedRows.includes(item.id)));
                setSelectedRows([]);
      ...
      <div className="space-y-6">
        <div className="flex justify-between items-start animate-fadeInDown">
          <div>
            ...
            <p className="text-blue-700 mt-2 text-md">Manage your prepared items</p>
          </div>
          <Button
            onClick={startCreate}
        ...
        <div className="flex items-center gap-2 p-1 bg-white/50 backdrop-blur-sm rounded-2xl border border-white/20 shadow-sm w-fit animate-fadeInDown">
          {[
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs font-bold transition-all ${productionFilter === tab.id
        ...
        {(isCreating || isEditing) && (
          <div className="bg-white/90 backdrop-blur-sm rounded-2xl border-2 border-white p-6 shadow-lg hover:shadow-xl transition-all animate-slideInUp">
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-semibold text-foreground">{isEditing ? "Edit Prepared Item" : "Add Prepared Item"}</h2>
              <button
                onClick={cancelForm}
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <FormSelect
                  ...
                  options={categories.map(cat => ({ value: cat.category_name, label: getCategoryLabel(cat.category_name) }))}
                />
                <FormInput
                  ...
                  onChange={(e) => {
                    ...
                    const total = (Number(qty) * Number(formData.pricePerItem)).toFixed(2);
                    setFormData({ ...formData, quantityPrepared: qty, totalPrice: total });
                  }}
                ...
                <FormInput
                  ...
                  onChange={(e) => {
                    ...
                    const total = (Number(price) * Number(formData.quantityPrepared)).toFixed(2);
                    setFormData({ ...formData, pricePerItem: price, totalPrice: total });
                  }}
              ...
              />
              <div className="mt-4">
                <label className="block text-sm font-medium text-foreground mb-2">Groceries Used</label>
                <div className="space-y-2">
                  {formData.groceriesUsed.map((g, idx) => (
                    <div key={idx} className="flex flex-col md:flex-row gap-2 items-end md:items-center">
                      <div className="flex-1">
                        <FormSelect
                          label="Grocery"
                          value={g.groceryId}
                          onChange={e => handleGroceryChange(idx, "groceryId", e.target.value)}
                          error={errors[`grocery_${idx}`]}
                          options={groceryMaster.map(gm => ({ value: gm.id, label: gm.name }))}
                        />
                      </div>
                      <div className="flex-1">
                        <FormInput
                          ...
                          step="0.01"
                          value={g.quantity}
                          onChange={e => handleGroceryChange(idx, "quantity", e.target.value)}
                      ...
                      <div>
                        {formData.groceriesUsed.length > 1 && (
                          <button type="button" onClick={() => handleRemoveGrocery(idx)} className="text-red-600 hover:text-red-700 p-2">
                  ...
                  <button type="button" onClick={handleAddGrocery} className="text-primary hover:underline text-sm mt-2">
                    + Add Grocery
                  </button>
                </div>
              </div>
              <div className="flex justify-end gap-3 mt-4">
                <Button type="button" variant="outline" onClick={cancelForm} className="bg-white">Cancel</Button>
        ...
        <div className="bg-white/80 backdrop-blur-sm rounded-2xl border-2 border-white p-6 shadow-lg hover:shadow-xl transition-all animate-slideInUp">
          ...
          <DataTable
            ...
            columns={[
              ...
              { key: "pricePerItem", label: "Price/Item", render: v => `₹${Number(v).toFixed(2)}` },
              { key: "totalPrice", label: "Total Price", render: v => `₹${Number(v).toFixed(2)}` },
              { key: "description", label: "Description" },
              {
                key: "preparedDate", label: "Prepared Date", render: v => {
                  if (!v) return "";
                  const d = new Date(v);
              ...
              },
              {
                ...
                render: (value) => (
                  <ul className="text-xs list-disc pl-4">
                    {Array.isArray(value) ? value.map((g, i) => (
                      <li key={i}>{g.groceryName} - {g.quantity} {g.unit}</li>
                    )) : value}
                  </ul>
                )
              },
